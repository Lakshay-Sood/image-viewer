/**
 *
 * @param {Number} startIndex start index of data array
 * @param {Number} size size of data array
 * @returns {{status, data: [{previewImage: string, title: string}]}}
 */
const fetchData = (startIndex, size) => {
	const totalData = [
		{
			previewImage:
	...
	const endIndex = Math.min(startIndex + size, totalData.length);
	return {
		status: 'passed',
